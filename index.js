...
function getArtistByIndex(arr,number) {
  let thisone = arr[number]["name"];
  let string = `the artist at index ${number} is ${thisone}`
return string;
}
...
function listOfNames(arr) {
  let theirNamesDoe = arr;
   for (let i = 0; i < theirNamesDoe.length; i++){
 let element = theirNamesDoe[i];
 theirNamesDoe[i] = element["name"];
   }
 return theirNamesDoe;
 }
...
function removeArtist(arr,number) {
  let oneGottaGo = arr;
  let theOneGone = oneGottaGo.shift(a);
  let a = number;
  return oneGottaGo;
}
...
newArtistOnTheBlock.push(risingTalent);
return newArtistOnTheBlock;
}
//console.log(addArtist,"John Doe", "1988-2022", "Full Stack Development", "African American", "I have a background in customer service at Big Retail Chain. I am attending BloomTech to become a Frontend Developer.");
...
let specificNationality = name.filter(artist => {
  let artistNationality = artist["nationality"];
  let artistNationalityarr = artistNationality.split(",");
  if(artistNationalityarr.length === 1 && artistNationality === nationality){
  return true;
  }
  else {
   return false;
...
for (let i = 0; i < specificNationality.length; i++) {
  const element = specificNationality[i];
  specificNationality[i] = element["name"]
  }
